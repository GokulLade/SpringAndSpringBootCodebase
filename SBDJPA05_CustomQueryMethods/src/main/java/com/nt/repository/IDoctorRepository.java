...
//	public List<Doctor> showDoctorByQlyPara2(String qly1, String qly2) throws Exception;
	...
	@Query(" Select docid,docName, income from Doctor where where docName=:dName") // Single row scalar query [Multiple
																					// columns]
	...
	@Query("Select distinct count(docName) from Doctor")
	public Long showUniqueDoctorCount() throws Exception;

	@Query("Select count(*),max(income),min(income),sum(income),avg(income) from Doctor")
	public Object showDoctorAggregateData() throws Exception;
	
	
	/* ----------------------- Non-Select Operations --------------------------------------*/
	@Query("update Doctor set income=:newIncome where qly=:qly1")
	@Modifying
	@Transactional
	public int updateDoctorIncomeByQly(String qly1,Double newIncome) throws Exception;
	
	@Query("delete from Doctor where income>=:startIncome and income<=:endIncome")
	@Modifying
	@Transactional
	public int removeDoctorByIncomeRange(Double startIncome,Double endIncome) throws Exception;
	
	
	/* ----------------------- Native SQL Query(Original Sql Query) --------------------------------------*/
	@Query(value="Select now() from dual",nativeQuery = true)
	public Timestamp showSystemDateAndTime() throws Exception;
