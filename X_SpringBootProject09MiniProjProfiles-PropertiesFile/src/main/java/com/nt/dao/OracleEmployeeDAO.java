...
public class OracleEmployeeDAO implements IEmployeeDAO{
	...
	//SQL Query
	private static final String GET_EMP_QUERY="SELECT EMPNO,ENAME,desg,salary,DEPTNO FROM EMP WHERE desg IN(?,?,?) ORDER BY desg";
	private static final String INSERT_EMP_DATA_QUERY="INSERT INTO EMP(EMPNO,ENAME,DESG,SALARY,DEPTNO,GROSSSALARY,NETSALARY) VALUES(EMP_ID_SEQ01.nextval,?,?,?,?,?,?)";
	
	//For Checking object is created or not
	public OracleEmployeeDAO() 
	{
		System.out.println("OracleEmployeeDAO.OracleEmployeeDAO()");
	}
